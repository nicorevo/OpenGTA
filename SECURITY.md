...
| Richieste geocoding | `src/app/geocode.ts`, `src/app/live-controls.ts` | endpoint Nominatim (OSM) pinnato a livello di compilazione, mai derivato da input utente (la query è serializzata con `URLSearchParams`); richiesta solo per azione esplicita: debounce 400 ms, min 2 caratteri, 1 richiesta in-flight con `AbortController` (le nuove ricerche abortano la precedente); timeout 5 s; budget 256 KiB via `readBoundedJson`; validazione per-candidato (lat ±90 / lon ±180 finiti, `display_name` ≤ 256 caratteri trimmed, `place_id` presente), i candidati non validi sono scartati; cache LRU in memoria (32 query normalizzate, solo i successi, mai i fallimenti); il testo dei candidati entra nel DOM solo via `textContent`; in modalità Offline il campo è disabilitato e non parte alcuna richiesta |
