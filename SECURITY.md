...
| Richieste geocoding | `src/app/geocode.ts`, `src/app/live-controls.ts`, `src/app/place-status.ts`, `src/app/bootstrap.ts` | endpoint Nominatim (OSM) pinnati a livello di compilazione (`/search` + `/reverse`), mai derivati da input utente (query e coordinate serializzate con `URLSearchParams`); la search parte solo per azione esplicita: debounce 400 ms, min 2 caratteri, 1 richiesta in-flight con `AbortController` (le nuove ricerche abortano la precedente); il reverse parte solo al cambio di zona (cella 1000 m, intervallo minimo 5 s tra richieste, 1 in-flight, abort solo su `dispose`), a riposo zero richieste; timeout 5 s e budget 256 KiB via `readBoundedJson` per entrambi; validazione (lat ±90 / lon ±180 finiti, `display_name` ≤ 256 caratteri trimmed, `place_id` presente), i candidati non validi sono scartati; `{error}` del reverse = esito legittimo "nessun dato"; cache LRU in memoria (32 query normalizzate, solo i successi, mai i fallimenti) solo per la search, nessuna per il reverse; i testi dei candidati e del luogo corrente entrano nel DOM solo via `textContent`; in modalità Offline il campo è disabilitato e non parte alcuna richiesta (anche di reverse) |
...
| Canvas e DOM | `src/render/pixi/*`, `bootstrap.ts`, `live-controls.ts`, `touch-controls.ts` | tutto il testo applicativo passa da `textContent`; nessun `innerHTML`, `document.write`, `eval` o `new Function` nel codice applicativo; PixiJS non carica texture, font o asset da URL remoti |
