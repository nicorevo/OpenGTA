...
/** Pinned, trusted geocoding endpoint (never taken from user input). */
...
export interface GeocodeClientOptions {
  /** Defaults to the pinned Nominatim URL; injectable for tests. */
...
export function createGeocodeClient(options: GeocodeClientOptions = {}): GeocodeClient {
  ...
  return {
    async search(query, callerSignal) {
      ...
      try {
        if (callerSignal?.aborted || controller.signal.aborted) throw new GeocodeError("aborted", "geocode search aborted");
        const response = await fetcher(url.toString(), { signal: controller.signal });
        if (!response.ok) {
          if (response.status === 429) throw new GeocodeError("rate-limited", "geocode search rate limited", response.status);
          throw new GeocodeError("http", "geocode request failed", response.status);
        }
        const value = await readBoundedJson(response, controller.signal, maxResponseBytes);
