...
import { stepVehicle, type VehicleState } from "../gameplay/vehicle/controller.ts";
...
export async function bootstrap(root: HTMLElement): Promise<void> {
  ...
  const keys = new Set<string>(); let vehicle: VehicleState = { position: { x: 0, y: 0 }, velocity: { x: 0, y: 0 }, heading: 0 }; window.addEventListener("keydown", (event) => { keys.add(event.key.toLowerCase()); if (event.key === "F3") { overlay.hidden = !overlay.hidden; overlay.textContent = `region: ${region.id}\nbuildings: ${region.buildings.length}\nroads: ${region.roads.length}\ncompiled: ${result.diagnostics.compiledFeatureCount}\nphysics colliders: ${physics.colliderCount()}\nwarnings: ${region.warnings.length + result.diagnostics.warnings.length}\nrenderer: PixiJS WebGL`; } }); window.addEventListener("keyup", (event) => keys.delete(event.key.toLowerCase())); let last = performance.now(); const frame = (now: number) => { const dt = Math.min(0.25, (now - last) / 1000); last = now; vehicle = stepVehicle(vehicle, { throttle: keys.has("w") || keys.has("arrowup") ? 1 : keys.has("s") || keys.has("arrowdown") ? -1 : 0, steer: keys.has("a") || keys.has("arrowleft") ? -1 : keys.has("d") || keys.has("arrowright") ? 1 : 0, brake: keys.has(" ") ? 1 : 0 }, Math.min(dt, 1 / 30)); requestAnimationFrame(frame); }; requestAnimationFrame(frame);
}
