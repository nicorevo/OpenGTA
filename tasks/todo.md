...
- [x] VPS-13: integrazione client OpenGTA (spec §106) — `src/vps/client/` (profile client fire-and-forget per cella h3 con TTL/cooldown/timeout 60 s, runtime guard sul body, `?vpsService=` a registry chiuso: dev locale / prod https) + wiring bootstrap (~20 righe, `serviceProfile ?? fixture ?? LVP`, `__opengtaV0Debug.vps()`); 19 unit + 3 e2e (mockati) + **smoke con servizio reale** cache fredda: tema LVP `rome` → `vps:v1:h3:891e8052a6bffff:c1` in ~30 s di background (generated), warm 1,5 ms, zero pageerror. Gate: 744/744 unit, e2e 46+1 skip. Dettaglio in `docs/results/VISUAL-PROFILE-SERVICE-V1-RESULT.md` (sezione VPS-13).
