...
| [x] | VPS-10 | VPS-09 | L | Runtime API `GET /v1/profile` (spec §106, §54 lvl 2-3, §82-83, §110): servizio Node (TS nativo, zero dipendenze) — `createOsmSource` Overpass (query con vocabolario collector, lunghezze/perimetri da `out geom`, edifici single-way chiusi e multipolygon con area shoelace, errori → throw per degrado §80); `createMapillaryClient` sulla **API correnti** (recon 2026-09-22: `graph.mapillary.com`, Bearer, `GET /images?bbox=…` — la v1 REST documentata non esiste più; detections senza label → `[]` §31-32); `createFileValueCache` (file JSON TTL, write atomiche) + adapter `EvidenceCache`/`ProfileCache` su disco → freschezza §54 livelli 2-3 (entry servizio per cella, hit senza rete, sopravvive al restart); token bucket rate limit clock iniettabile; handler `GET /v1/profile?lat&lon` → 200 generated/cache/**lvp** (fallback LVP immediato, mai 5xx per provider, 400 coordinate, 404 path, CORS); parent per cerchio di città (Rome/Paris/Tokyo) → `THEME_BY_ID`; analyzer = test (vision modello = VPS-11); fix core `surface=sett` → cobblestone (dati live Roma). `service/` + `.env.example` + script `npm run service`, 36 test + 1 regressione |
| [x] | VPS-11 | VPS-10 | M | Modello vision server-side + misurazione costo/latenza (spec §107, §18-21/30/78/82-83): `createDeepSeekAnalyzer` (scelta utente: DeepSeek `deepseek-flash`, unico modello vision della piattaforma; OpenAI-compatible `api.deepseek.com`) dietro il contratto `VisualAnalyzer` — l'output grezzo passa sempre lo stesso validatore stretto §30/78 (prompt con i vocabolari **importati** dal validatore, zero drift §21); risposta malformata → fallback quality 0 (mai throw), fallimento di servizio (HTTP/immagine/envelope) → throw → degrado pipeline §80; **due quirk live verificati**: egress DeepSeek non raggiunge la CDN Mapillary → thumbnail scaricate dal servizio (guardie: 8 MiB, content-type image/*, timeout 30 s) e inviate base64, memo in-memory condivisa (spec 17: niente persistenza immagini); thinking mode default ON brucia il budget di completion → `thinking:{type:"disabled"}` + temperature 0; `VisionStats` per generazione in response (requests/analyzed/fallbacks/totalMs/tokens/estimatedCostUsd a tassi peak) = misurazione §107; `DEEPSEEK_API_KEY` opzionale in `.env` (senza chiave → OSM-only invariato). `service/vision/`, 21 test (15 analyzer + 4 env + 2 server) |
| [x] | VPS-12 | VPS-11 | M | Coverage QA (spec §108, §109 parziale, §112-113): 7 generazioni live su Roma — centro (Piazza Navona: 23 img → 4 selezionate dal cap di diversità §14, historic-dense), periferia (cella con copertura: **suburban** ✓), industriale (cella edge del centro antico: historic-dense, semanticamente accettabile), suburbano/planned (EUR: **suburban** ✓ + OSM failed → degrado corretto), scarsa imagery terra (0 img nella cella esatta) e estremo (mare: profilo generato **identico al parent** per tutti i colori), più 1 cella "vision down" (chiave DeepSeek invalida: 200, vision {8 req / 0 analyzed / 8 errors / $0}, roads da OSM cobblestone). **Finding chiave**: copertura Mapillary a granularità di cella (h3 res 9 ≈ 500 m) **patchy** — 4/6 celle iniziali a 0 img nella cella esatta (verificato con bbox esatti contro l'API: 27-29 img nel km circostante): rischio NO-GO §110 "imagery insufficiente" mitigato per design dal parent/LVP (VPS enhances, LVP guarantees). Matrice fallback live: OSM down / imagery assente / vision down / tutto assente → mai 5xx. Costi $0.0014-0.0045/cella con vision, $0 senza; cache hit ~19 ms zero scritture corpo identico; determinismo pipeline a parità di input (il provider live può variare il set di immagini tra chiamate: 13/8/10 sul Colosseo — revisioni diverse, documentato). Cambio di codice: `VisionStats.errors` (fallimenti di servizio misurati; `fallbacks` resta per risposte rifiutate dal validatore), 7 asserzioni. Unit 725/725, build ok |
| [x] | VPS-GATE-V1 | VPS-12 | M | Gate VPS v1 (spec §109, §110-112): 3 generazioni live (Roma Colosseo 29,2 s osm-failed, Parigi 27,8 s, Tokyo 36,5 s). (1) città distinte: 45-48/51 campi numerici pairwise, distanze euclidee 0,51-1,01 ✓; (2) > country theme: 6-8 campi evidence-driven (`vegetation` sparse vs temperate-urban conf 1,0 tokyo; marciapiedi paris asfalto 0,72), assi deboli pinnati sul parent ✓ conservativo; (3) determinismo: 50/53 campi su 2 rigenerazioni fresh, 3 diversi — causa radice: Mapillary restituisce set di campioni diverso a ogni chiamata (10 vs 6), identità stabile (entrambe historic-dense), mitigato da cache TTL 7 gg (v2: campionamento stabile) ✓ con caveat documentato; (4) costo $0,0022-0,0034/cella, $0 senza imagery ✓; (5) cache hit 1,5 ms vs 29-36 s ✓; (6) provider failure: matrice VPS-12 + run gate con osm failed → mai 5xx ✓; (7) QA visuale §112: e2e `vps-visual-profiles.spec.ts` (rendering deterministico: 0,00% su load identici) — vps=rome vs fallback 9,1% (soglia >4%), vps=paris 12,9% (soglia >7%) ✓. Campionatura §110: griglia h3 217 celle raggio ~5 km → **68,7%** copertura (1/1 centro → 32/48 anello 8): NO-GO "imagery insufficiente" **non innescato** (celle senza dati → profilo identico al parent, by design). **Verdetto: GO**. Gate: unit 725/725, build ok, e2e 43+1 skip |
| [x] | VPS-13 | VPS-GATE-V1 | M | Integrazione client OpenGTA (spec §106 "Integrare OpenGTA", §54 lvl 1, §80): `src/vps/client/` puro (fetch/clock iniettati) — `createVpsProfileClient`: `sync(location)` fire-and-forget su cella h3 res 9 (stessa cella del servizio), dedup in-flight, cache client per cella TTL 10 min, cooldown 60 s sul fallimento (mai hammering dal tick 200 ms), timeout fetch 60 s (copre la generazione fredda osservata 29-36 s); applica il profilo solo per `source: generated|cache` (per `lvp` il client mantiene la propria risoluzione LVP, più granulare); cella nuova ≠ cella applicata → profilo scartato subito (fallback LVP immediato, nessun bleed cross-città); runtime guard stretta sul body (`isVisualProfile`, input non attendibile al confine); `diagnostics()` in `__opengtaV0Debug.vps()`. URL servizio: `?vpsService=` opzionale, registry chiuso (dev: origini locali; prod: https; mai credenziali/query/hash), assente = VPS spento (zero impatto offline/esistente), hook `?vps=` (dev) ha precedenza (un solo source alla volta). `bootstrap.ts`: client solo se open-world + param + nessun fixture; `syncVisualTheme`: serviceProfile ?? fixture ?? LVP; fetch avvolto (non destrutturato: riferimento nudo = `Illegal invocation`, trovato in e2e). 19 unit (client 11 + service-url 8) + e2e 3 (mockato: generated → id vps applicato; 500 → LVP + failed + 1 sola attempt; assente → zero richieste) + **smoke servizio reale** cache fredda (Colosseo): rome → `vps:v1:h3:891e8052a6bffff:c1` a ~30 s (generated), warm 1,5 ms, zero pageerror. Gate: unit 744/744, typecheck, build, e2e 46+1 skip |
