...
8. Use `.opencode/references/performance-checklist.md` as the minimum baseline for each area.
...
- **Do not invoke from another persona.** If `code-reviewer` flags a performance concern that warrants a deeper pass, surface that recommendation in the report; the user or the main agent initiates the deeper pass. See [agent routing](AGENTS.md).
