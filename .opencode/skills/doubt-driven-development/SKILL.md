...
- **Do NOT add this skill to a persona's `skills:` frontmatter.** A persona that follows Step 3 would spawn another persona. Personas must produce their own perspective; orchestration belongs to the main session or to an explicit user-driven command.
...
- **Repo orchestration rule:** this skill orchestrates from the main session. A persona calling another persona is not allowed — see Loading Constraints above.
